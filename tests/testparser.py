...
for base, dirs, files in os.walk(args.source_dir[0]):
    for f in filter(lambda p: p.endswith(".rs"), files):
        ...
        for parser in args.parser:
            if (
                subprocess.call(
                    parser, stdin=open(p), stderr=subprocess.STDOUT, stdout=devnull
                )
                == 0
            ):
                if parse_fail:
                ...
                if parse_fail:
                    ...
                    bad[parser].append(p)
        parser_stats = ", ".join(
            ["{}: {}".format(parser, ok[parser]) for parser in args.parser]
        )
        sys.stdout.write(
            "\033[K\r total: {}, {}, scanned {}".format(
                total, os.path.relpath(parser_stats), os.path.relpath(p)
            )
        )
...
for parser in args.parser:
    ...
    print(
        "writing {} files that did not yield the correct result with {} to {}".format(
            len(bad[parser]), parser, filename
        )
    )
